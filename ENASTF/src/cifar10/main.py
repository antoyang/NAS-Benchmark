...
from src.cifar10.data_utils import read_data, read_data_large
...
def train():
  if FLAGS.child_fixed_arc is None:
    images, labels = read_data(FLAGS.data_path, FLAGS.dataset)
  else:
  ...
  with g.as_default():
    ...
    checkpoint_saver_hook = tf.train.CheckpointSaverHook(FLAGS.output_dir, save_steps=child_ops["num_train_batches"], saver=saver)
