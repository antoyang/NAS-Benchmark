...
parser.add_argument('--path', type=str,default="/antoine/nas-benchmark/NSGANET")
...
from models.micro_models import PyramidNetworkCIFAR as PyrmNASNet, NetworkImageNet
...
def main():
    ...
    if args.dataset == "CIFAR10":
        ...
        train_transform, valid_transform = utils._data_transforms_large(args)
        train_data = torchvision.datasets.ImageFolder(root=args.data+"/"+args.dataset+"/train", transform=train_transform)
        valid_data = torchvision.datasets.ImageFolder(root=args.data+"/"+args.dataset+"/test", transform=valid_transform)
    ...
    if args.net_type == 'micro':
        ...
        if args.dataset=="Sport8":
            net = NetworkImageNet(args.init_channels, num_classes=8, layers=args.layers, auxiliary=args.auxiliary, genotype=genotype)
        elif args.dataset=="MIT67":
            net = NetworkImageNet(args.init_channels, num_classes=67, layers=args.layers, auxiliary=args.auxiliary, genotype=genotype)
        elif args.dataset=="flowers102":
            net = NetworkImageNet(args.init_channels, num_classes=102, layers=args.layers, auxiliary=args.auxiliary, genotype=genotype)
