...
def main():
    ...
    if config.label_smooth!=0:
        criterion = utils.CrossEntropyLabelSmooth(10, config.label_smooth).to(device)
    else:
    ...
    if config.dataset in utils.LARGE_DATASETS:
        ...
        model = AugmentCNN(input_size, input_channels, config.init_channels, n_classes, config.layers,
                           use_aux, config.genotype, SSC=config.SSC)
    ...
    # weights optimizer
    if config.p!=1:
        optimizer = torch.optim.SGD(model.parameters(), 1., momentum=config.momentum,
    ...
    if config.p==1:
        ...
        lr_cpa = utils.cosine_power_annealing_lr(nepochs=config.epochs, min_lr=config.lr_min, max_lr=config.lr, p=config.p)
